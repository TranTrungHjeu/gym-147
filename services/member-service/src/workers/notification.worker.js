...
/**
 * Notification Worker for Member Service
...
class NotificationWorker {
  ...
  async processNotification(notificationData) {
    try {
      ...
      // Send notification to identity service (or create directly if we have access)
      // For now, we'll send push notification directly
      ...
      if (global.io) {
        global.io.to(`user:${actualUserId}`).emit('notification:new', {
          type,
          title,
          message,
          data,
