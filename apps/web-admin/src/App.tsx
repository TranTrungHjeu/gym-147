@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@300;400;500;600;700&display=swap');
/*
  ---break---

  ---break---
*/

@import 'tailwindcss';

/* Custom animations for class cards */
@keyframes fadeInUp {
  from {
    opacity: 0;
    transform: translateY(30px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}

.animate-fadeInUp {
  animation: fadeInUp 0.6s ease-out forwards;
}

/* Smooth animations for real-time updates */
@keyframes slideInFromRight {
  from {
    opacity: 0;
    transform: translateX(20px);
  }
  to {
    opacity: 1;
    transform: translateX(0);
  }
}

@keyframes highlightUpdate {
  0% {
    background-color: transparent;
  }
  50% {
    background-color: rgba(59, 130, 246, 0.1); /* blue-500/10 */
  }
  100% {
    background-color: transparent;
  }
}

@keyframes pulseUpdate {
  0%,
  100% {
    box-shadow: 0 0 0 0 rgba(59, 130, 246, 0.4);
  }
  50% {
    box-shadow: 0 0 0 4px rgba(59, 130, 246, 0);
  }
}

.animate-slide-in-right {
  animation: slideInFromRight 0.4s ease-out forwards;
}

.animate-highlight-update {
  animation: highlightUpdate 1.5s ease-in-out forwards;
}

.animate-pulse-update {
  animation: pulseUpdate 1s ease-in-out forwards;
}

/* Smooth notification slide-in */
@keyframes notificationSlideIn {
  from {
    opacity: 0;
    transform: translateX(100%) scale(0.95);
  }
  to {
    opacity: 1;
    transform: translateX(0) scale(1);
  }
}

.animate-notification-slide-in {
  animation: notificationSlideIn 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

/* Smooth schedule row update */
@keyframes scheduleRowUpdate {
  0% {
    background-color: transparent;
    transform: scale(1);
  }
  50% {
    background-color: rgba(34, 197, 94, 0.1); /* green-500/10 */
    transform: scale(1.01);
  }
  100% {
    background-color: transparent;
    transform: scale(1);
  }
}

.schedule-row-updated {
  animation: scheduleRowUpdate 1s ease-in-out forwards;
}

/* Smooth booking count update */
@keyframes countUpdate {
  0% {
    transform: scale(1);
  }
  50% {
    transform: scale(1.2);
    color: rgb(34, 197, 94); /* green-500 */
  }
  100% {
    transform: scale(1);
  }
}

.animate-count-update {
  animation: countUpdate 0.5s ease-out forwards;
}

/* Smooth certification deletion animation */
@keyframes certificationFadeOut {
  0% {
    opacity: 1;
    transform: scale(1);
    max-height: 200px;
    margin-bottom: 8px;
  }
  100% {
    opacity: 0;
    transform: scale(0.95);
    max-height: 0;
    margin-bottom: 0;
    padding-top: 0;
    padding-bottom: 0;
  }
}

.certification-deleting {
  animation: certificationFadeOut 0.4s ease-in forwards;
  overflow: hidden;
}

/* SUPER_ADMIN badge styling */
.super-admin-badge-container {
  position: relative;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  padding: 2px;
  border-radius: 9999px;
}

.super-admin-badge-back {
  position: relative;
  width: 100%;
  height: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  overflow: hidden;
  border-radius: 9999px;
}

.super-admin-badge-content {
  position: relative;
  z-index: 1;
  background: inherit;
  border-radius: 9999px;
}

@keyframes gradientFlowHorizontal {
  0% {
    background-position: 200% center;
  }
  50% {
    background-position: -200% center;
  }
  100% {
    background-position: 200% center;
  }
}

@custom-variant dark (&:is(.dark *));

@theme {
  --font-*: initial;
  --font-space-grotesk: 'Space Grotesk', sans-serif;
  --font-inter: 'Inter', sans-serif;
  --font-display: 'Space Grotesk', sans-serif;
  --font-heading: 'Space Grotesk', sans-serif;

  --breakpoint-*: initial;
  --breakpoint-2xsm: 375px;
  --breakpoint-xsm: 425px;
  --breakpoint-3xl: 2000px;
  --breakpoint-sm: 640px;
  --breakpoint-md: 768px;
  --breakpoint-lg: 1024px;
  --breakpoint-xl: 1280px;
  --breakpoint-2xl: 1536px;

  --text-title-2xl: 72px;
  --text-title-2xl--line-height: 90px;
  --text-title-xl: 60px;
  --text-title-xl--line-height: 72px;
  --text-title-lg: 48px;
  --text-title-lg--line-height: 60px;
  --text-title-md: 36px;
  --text-title-md--line-height: 44px;
  --text-title-sm: 30px;
  --text-title-sm--line-height: 38px;
  --text-theme-xl: 20px;
  --text-theme-xl--line-height: 30px;
  --text-theme-sm: 14px;
  --text-theme-sm--line-height: 20px;
  --text-theme-xs: 12px;
  --text-theme-xs--line-height: 18px;

  --color-current: currentColor;
  --color-transparent: transparent;
  --color-white: #ffffff;
  --color-black: #101828;

  --color-brand-25: #f2f7ff;
  --color-brand-50: #ecf3ff;
  --color-brand-100: #dde9ff;
  --color-brand-200: #c2d6ff;
  --color-brand-300: #9cb9ff;
  --color-brand-400: #7592ff;
  --color-brand-500: #465fff;
  --color-brand-600: #3641f5;
  --color-brand-700: #2a31d8;
  --color-brand-800: #252dae;
  --color-brand-900: #262e89;
  --color-brand-950: #161950;

  --color-blue-light-25: #f5fbff;
  --color-blue-light-50: #f0f9ff;
  --color-blue-light-100: #e0f2fe;
  --color-blue-light-200: #b9e6fe;
  --color-blue-light-300: #7cd4fd;
  --color-blue-light-400: #36bffa;
  --color-blue-light-500: #0ba5ec;
  --color-blue-light-600: #0086c9;
  --color-blue-light-700: #026aa2;
  --color-blue-light-800: #065986;
  --color-blue-light-900: #0b4a6f;
  --color-blue-light-950: #062c41;

  --color-gray-25: #fcfcfd;
  --color-gray-50: #f9fafb;
  --color-gray-100: #f2f4f7;
  --color-gray-200: #e4e7ec;
  --color-gray-300: #d0d5dd;
  --color-gray-400: #98a2b3;
  --color-gray-500: #667085;
  --color-gray-600: #475467;
  --color-gray-700: #344054;
  --color-gray-800: #1d2939;
  --color-gray-900: #101828;
  --color-gray-950: #0c111d;
  --color-gray-dark: #1a2231;

  --color-orange-25: #fffaf5;
  --color-orange-50: #fff6ed;
  --color-orange-100: #ffead5;
  --color-orange-200: #fddcab;
  --color-orange-300: #feb273;
  --color-orange-400: #fd853a;
  --color-orange-500: #fb6514;
  --color-orange-600: #ec4a0a;
  --color-orange-700: #c4320a;
  --color-orange-800: #9c2a10;
  --color-orange-900: #7e2410;
  --color-orange-950: #511c10;

  --color-success-25: #f6fef9;
  --color-success-50: #ecfdf3;
  --color-success-100: #d1fadf;
  --color-success-200: #a6f4c5;
  --color-success-300: #6ce9a6;
  --color-success-400: #32d583;
  --color-success-500: #12b76a;
  --color-success-600: #039855;
  --color-success-700: #027a48;
  --color-success-800: #05603a;
  --color-success-900: #054f31;
  --color-success-950: #053321;

  --color-error-25: #fffbfa;
  --color-error-50: #fef3f2;
  --color-error-100: #fee4e2;
  --color-error-200: #fecdca;
  --color-error-300: #fda29b;
  --color-error-400: #f97066;
  --color-error-500: #f04438;
  --color-error-600: #d92d20;
  --color-error-700: #b42318;
  --color-error-800: #912018;
  --color-error-900: #7a271a;
  --color-error-950: #55160c;

  --color-warning-25: #fffcf5;
  --color-warning-50: #fffaeb;
  --color-warning-100: #fef0c7;
  --color-warning-200: #fedf89;
  --color-warning-300: #fec84b;
  --color-warning-400: #fdb022;
  --color-warning-500: #f79009;
  --color-warning-600: #dc6803;
  --color-warning-700: #b54708;
  --color-warning-800: #93370d;
  --color-warning-900: #7a2e0e;
  --color-warning-950: #4e1d09;

  --color-theme-pink-500: #ee46bc;

  --color-theme-purple-500: #7a5af8;

  --shadow-theme-md: 0px 4px 8px -2px rgba(16, 24, 40, 0.1), 0px 2px 4px -2px rgba(16, 24, 40, 0.06);
  --shadow-theme-lg: 0px 12px 16px -4px rgba(16, 24, 40, 0.08),
    0px 4px 6px -2px rgba(16, 24, 40, 0.03);
  --shadow-theme-sm: 0px 1px 3px 0px rgba(16, 24, 40, 0.1), 0px 1px 2px 0px rgba(16, 24, 40, 0.06);
  --shadow-theme-xs: 0px 1px 2px 0px rgba(16, 24, 40, 0.05);
  --shadow-theme-xl: 0px 20px 24px -4px rgba(16, 24, 40, 0.08),
    0px 8px 8px -4px rgba(16, 24, 40, 0.03);
  --shadow-datepicker: -5px 0 0 #262d3c, 5px 0 0 #262d3c;
  --shadow-focus-ring: 0px 0px 0px 4px rgba(70, 95, 255, 0.12);
  --shadow-slider-navigation: 0px 1px 2px 0px rgba(16, 24, 40, 0.1),
    0px 1px 3px 0px rgba(16, 24, 40, 0.1);
  --shadow-tooltip: 0px 4px 6px -2px rgba(16, 24, 40, 0.05),
    -8px 0px 20px 8px rgba(16, 24, 40, 0.05);

  --drop-shadow-4xl: 0 35px 35px rgba(0, 0, 0, 0.25), 0 45px 65px rgba(0, 0, 0, 0.15);

  --z-index-1: 1;
  --z-index-9: 9;
  --z-index-99: 99;
  --z-index-999: 999;
  --z-index-9999: 9999;
  --z-index-99999: 99999;
  --z-index-999999: 999999;
}

/*
  The default border color has changed to `currentColor` in Tailwind CSS v4,
  so we've added these compatibility styles to make sure everything still
  looks the same as it did with Tailwind CSS v3.

  If we ever want to remove these styles, we need to add an explicit border
  color utility to any element that depends on these defaults.
*/
@layer base {
  *,
  ::after,
  ::before,
  ::backdrop,
  ::file-selector-button {
    border-color: var(--border-color, currentColor);
  }
  button:not(:disabled),
  [role='button']:not(:disabled) {
    cursor: pointer;
  }
  body {
    @apply relative font-normal font-inter z-1;
    background-color: var(--bg-primary);
    color: var(--text-primary);
    transition: background-color 0.2s ease, color 0.2s ease;
  }

  /* Global scrollbar styling for entire application */
  html,
  body {
    scrollbar-width: thin;
    scrollbar-color: var(--color-neutral-300) transparent;
  }

  html::-webkit-scrollbar,
  body::-webkit-scrollbar {
    width: 6px;
    height: 6px;
  }

  html::-webkit-scrollbar-track,
  body::-webkit-scrollbar-track {
    background: transparent;
    border-radius: 6px;
  }

  html::-webkit-scrollbar-thumb,
  body::-webkit-scrollbar-thumb {
    background-color: var(--color-neutral-300);
    border-radius: 6px;
    transition: background-color 0.2s ease;
  }

  html::-webkit-scrollbar-thumb:hover,
  body::-webkit-scrollbar-thumb:hover {
    background-color: var(--color-neutral-400);
  }

  html::-webkit-scrollbar-corner,
  body::-webkit-scrollbar-corner {
    background: transparent;
  }

  /* Dark mode scrollbar */
  .dark html::-webkit-scrollbar-track,
  .dark body::-webkit-scrollbar-track {
    background: transparent;
  }

  .dark html::-webkit-scrollbar-thumb,
  .dark body::-webkit-scrollbar-thumb {
    background-color: var(--color-neutral-600);
  }

  .dark html::-webkit-scrollbar-thumb:hover,
  .dark body::-webkit-scrollbar-thumb:hover {
    background-color: var(--color-neutral-500);
  }

  .dark html,
  .dark body {
    scrollbar-color: var(--color-neutral-600) transparent;
  }

  /* Mobile responsive scrollbar */
  @media only screen and (max-width: 768px) {
    html::-webkit-scrollbar,
    body::-webkit-scrollbar {
      width: 4px;
      height: 4px;
    }

    html::-webkit-scrollbar-thumb,
    body::-webkit-scrollbar-thumb {
      border-radius: 4px;
    }

    html::-webkit-scrollbar-track,
    body::-webkit-scrollbar-track {
      border-radius: 4px;
    }
  }

  /* High DPI displays */
  @media screen and (min-resolution: 2dppx) {
    html::-webkit-scrollbar,
    body::-webkit-scrollbar {
      width: 5px;
      height: 5px;
    }
  }
}

/* ===== AUTH PAGES ANIMATIONS ===== */
@keyframes float {
  0%,
  100% {
    transform: translateY(0px);
  }
  50% {
    transform: translateY(-10px);
  }
}

@keyframes glow {
  0%,
  100% {
    box-shadow: 0 0 20px rgba(243, 97, 0, 0.3);
  }
  50% {
    box-shadow: 0 0 30px rgba(243, 97, 0, 0.6);
  }
}

@keyframes shimmer {
  0% {
    background-position: -200% 0;
  }
  100% {
    background-position: 200% 0;
  }
}

@keyframes selectHighlight {
  0% {
    background-color: rgba(251, 146, 60, 0.1);
    transform: scale(1);
  }
  50% {
    background-color: rgba(251, 146, 60, 0.2);
    transform: scale(1.01);
  }
  100% {
    background-color: rgba(251, 146, 60, 0.1);
    transform: scale(1);
  }
}

@keyframes buttonSlideIn {
  from {
    opacity: 0;
    transform: translateX(10px);
  }
  to {
    opacity: 1;
    transform: translateX(0);
  }
}

.animate-button-slide-in {
  animation: buttonSlideIn 0.3s ease-out forwards;
}

@keyframes slideInUp {
  from {
    opacity: 0;
    transform: translateY(30px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}

@keyframes slideInLeft {
  from {
    opacity: 0;
    transform: translateX(-30px);
  }
  to {
    opacity: 1;
    transform: translateX(0);
  }
}

@keyframes slideInRight {
  from {
    opacity: 0;
    transform: translateX(30px);
  }
  to {
    opacity: 1;
    transform: translateX(0);
  }
}

@keyframes fadeInScale {
  from {
    opacity: 0;
    transform: scale(0.9);
  }
  to {
    opacity: 1;
    transform: scale(1);
  }
}

/* Auth form animations */
.auth-form-enter {
  animation: slideInUp 0.6s ease-out;
}

.auth-form-field {
  animation: slideInUp 0.6s ease-out;
  animation-fill-mode: both;
}

.auth-form-field:nth-child(1) {
  animation-delay: 0.1s;
}
.auth-form-field:nth-child(2) {
  animation-delay: 0.2s;
}
.auth-form-field:nth-child(3) {
  animation-delay: 0.3s;
}
.auth-form-field:nth-child(4) {
  animation-delay: 0.4s;
}
.auth-form-field:nth-child(5) {
  animation-delay: 0.5s;
}

/* Floating elements */
.float-animation {
  animation: float 3s ease-in-out infinite;
}

.glow-animation {
  animation: glow 2s ease-in-out infinite;
}

/* Shimmer effect for buttons */
.shimmer {
  background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.2), transparent);
  background-size: 200% 100%;
  animation: shimmer 2s infinite;
}

/* Grid pattern background */
.bg-grid-pattern {
  background-image: linear-gradient(rgba(255, 255, 255, 0.1) 1px, transparent 1px),
    linear-gradient(90deg, rgba(255, 255, 255, 0.1) 1px, transparent 1px);
  background-size: 20px 20px;
}

/* Auth pages responsive optimizations */
@media only screen and (max-width: 768px) {
  .auth-form-container {
    padding: 1rem;
  }

  .auth-form-header {
    margin-bottom: 1.5rem;
  }

  .auth-form-title {
    font-size: 1.5rem;
  }

  .auth-form-subtitle {
    font-size: 0.875rem;
  }

  .auth-social-buttons {
    gap: 0.75rem;
  }

  .auth-form-fields {
    gap: 1rem;
  }

  .auth-form-button {
    padding: 0.75rem 1rem;
  }
}

@media only screen and (max-height: 600px) {
  .auth-form-container {
    padding: 0.75rem;
  }

  .auth-form-header {
    margin-bottom: 1rem;
  }

  .auth-form-title {
    font-size: 1.25rem;
  }

  .auth-form-subtitle {
    font-size: 0.75rem;
  }
}

/* Professional auth styling enhancements */
.auth-video-container {
  position: relative;
  overflow: hidden;
  border-radius: 0 0 0 1.5rem;
}

.auth-video-overlay {
  background: linear-gradient(
    135deg,
    rgba(0, 0, 0, 0.7) 0%,
    rgba(0, 0, 0, 0.3) 50%,
    rgba(0, 0, 0, 0.1) 100%
  );
}

.auth-brand-card {
  backdrop-filter: blur(20px);
  background: rgba(255, 255, 255, 0.1);
  border: 1px solid rgba(255, 255, 255, 0.2);
  box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.1);
}

.auth-feature-item {
  backdrop-filter: blur(10px);
  background: rgba(255, 255, 255, 0.08);
  border: 1px solid rgba(255, 255, 255, 0.15);
  transition: all 0.3s ease;
}

.auth-feature-item:hover {
  background: rgba(255, 255, 255, 0.12);
  border-color: rgba(255, 255, 255, 0.25);
  transform: translateY(-2px);
}

.auth-quote-card {
  backdrop-filter: blur(15px);
  background: rgba(255, 255, 255, 0.1);
  border: 1px solid rgba(255, 255, 255, 0.2);
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.1);
}

@utility menu-item {
  @apply relative flex items-center w-full gap-3 px-3 py-2 font-medium rounded-lg text-theme-sm;
}

@utility cta-button {
  @apply font-heading font-semibold tracking-wide;
}

@utility menu-item-active {
  @apply bg-brand-50 text-brand-500 dark:bg-brand-500/[0.12] dark:text-brand-400;
}

@utility menu-item-inactive {
  @apply text-gray-700 hover:bg-gray-100 group-hover:text-gray-700 dark:text-gray-300 dark:hover:bg-white/5 dark:hover:text-gray-300;
}

@utility menu-item-icon {
  @apply text-gray-500 group-hover:text-gray-700 dark:text-gray-400;
}

@utility menu-item-icon-active {
  @apply text-brand-500 dark:text-brand-400;
}

@utility menu-item-icon-size {
  & svg {
    @apply !size-6;
  }
}

@utility menu-item-icon-inactive {
  @apply text-gray-500 group-hover:text-gray-700 dark:text-gray-400 dark:group-hover:text-gray-300;
}

@utility menu-item-arrow {
  @apply relative;
}

@utility menu-item-arrow-active {
  @apply rotate-180 text-brand-500 dark:text-brand-400;
}

@utility menu-item-arrow-inactive {
  @apply text-gray-500 group-hover:text-gray-700 dark:text-gray-400 dark:group-hover:text-gray-300;
}

@utility menu-dropdown-item {
  @apply relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-theme-sm font-medium;
}

@utility menu-dropdown-item-active {
  @apply bg-brand-50 text-brand-500 dark:bg-brand-500/[0.12] dark:text-brand-400;
}

@utility menu-dropdown-item-inactive {
  @apply text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-white/5;
}

@utility menu-dropdown-badge {
  @apply block rounded-full px-2.5 py-0.5 text-xs font-medium uppercase text-brand-500 dark:text-brand-400;
}

@utility menu-dropdown-badge-active {
  @apply bg-brand-100 dark:bg-brand-500/20;
}

@utility menu-dropdown-badge-inactive {
  @apply bg-brand-50 group-hover:bg-brand-100 dark:bg-brand-500/15 dark:group-hover:bg-brand-500/20;
}
@utility no-scrollbar {
  /* Chrome, Safari and Opera */
  &::-webkit-scrollbar {
    display: none;
  }
  -ms-overflow-style: none; /* IE and Edge */
  scrollbar-width: none; /* Firefox */
}
@utility custom-scrollbar {
  scroll-behavior: smooth;
  overscroll-behavior: contain;
  scrollbar-width: thin;
  scrollbar-color: #d1d5db transparent;

  &::-webkit-scrollbar {
    width: 4px;
    height: 4px;
  }

  &::-webkit-scrollbar-track {
    background: transparent;
    border-radius: 4px;
  }

  &::-webkit-scrollbar-thumb {
    background-color: #d1d5db;
    border-radius: 4px;
    transition: background-color 0.2s ease;
  }

  &::-webkit-scrollbar-thumb:hover {
    background-color: #9ca3af;
  }

  &::-webkit-scrollbar-corner {
    background: transparent;
  }

  /* Dark mode adjustments */
  .dark & {
    scrollbar-color: #4b5563 transparent;

    &::-webkit-scrollbar-track {
      background: transparent;
    }

    &::-webkit-scrollbar-thumb {
      background-color: #4b5563;
    }

    &::-webkit-scrollbar-thumb:hover {
      background-color: #6b7280;
    }
  }
}

@layer utilities {
  /* For Remove Date Icon */
  input[type='date']::-webkit-inner-spin-button,
  input[type='time']::-webkit-inner-spin-button,
  input[type='date']::-webkit-calendar-picker-indicator,
  input[type='time']::-webkit-calendar-picker-indicator {
    display: none;
    -webkit-appearance: none;
  }
}

.tableCheckbox:checked ~ span span {
  @apply opacity-100;
}
.tableCheckbox:checked ~ span {
  @apply border-brand-500 bg-brand-500;
}

/* Custom checkbox color for orange theme */
input[type='checkbox'] {
  accent-color: #ea580c; /* orange-600 */
}

input[type='checkbox']:checked {
  background-color: #ea580c; /* orange-600 */
  border-color: #ea580c; /* orange-600 */
}

.dark input[type='checkbox']:checked {
  background-color: #ea580c; /* orange-600 */
  border-color: #ea580c; /* orange-600 */
}

/* third-party libraries CSS */
.apexcharts-legend-text {
  @apply !text-gray-700 dark:!text-gray-400;
}

.apexcharts-text {
  @apply !fill-gray-700 dark:!fill-gray-400;
}

.apexcharts-tooltip.apexcharts-theme-light {
  @apply gap-1 !rounded-lg !border-gray-200 p-3 !shadow-theme-sm dark:!border-gray-800 dark:!bg-gray-900;
}

.apexcharts-tooltip-marker {
  margin-right: 6px;
  height: 6px;
  width: 6px;
}
.apexcharts-legend-text {
  @apply !pl-5 !text-gray-700 dark:!text-gray-400;
}
.apexcharts-tooltip-series-group {
  @apply !p-0;
}
.apexcharts-tooltip-y-group {
  @apply !p-0;
}
.apexcharts-tooltip-title {
  @apply !mb-0 !border-b-0 !bg-transparent !p-0 !text-[10px] !leading-4 !text-gray-800 dark:!text-white/90;
}
.apexcharts-tooltip-text {
  @apply !text-theme-xs !text-gray-700 dark:!text-white/90;
}
.apexcharts-tooltip-text-y-value {
  @apply !font-medium;
}

.apexcharts-gridline {
  @apply !stroke-gray-100 dark:!stroke-gray-800;
}
#chartTwo .apexcharts-datalabels-group {
  @apply !-translate-y-24;
}
#chartTwo .apexcharts-datalabels-group .apexcharts-text {
  @apply !fill-gray-800 !font-semibold dark:!fill-white/90;
}

#chartDarkStyle .apexcharts-datalabels-group .apexcharts-text {
  @apply !fill-gray-800 !font-semibold dark:!fill-white/90;
}

#chartSixteen .apexcharts-legend {
  @apply !p-0 !pl-6;
}

.jvectormap-container {
  @apply !bg-gray-50 dark:!bg-gray-900;
}
.jvectormap-region.jvectormap-element {
  @apply !fill-gray-300 hover:!fill-brand-500 dark:!fill-gray-700 dark:hover:!fill-brand-500;
}
.jvectormap-marker.jvectormap-element {
  @apply !stroke-gray-200 dark:!stroke-gray-800;
}
.jvectormap-tip {
  @apply !bg-brand-500 !border-none !px-2 !py-1;
}
.jvectormap-zoomin,
.jvectormap-zoomout {
  @apply !hidden;
}

.stocks-slider-outer .swiper-button-next:after,
.stocks-slider-outer .swiper-button-prev:after {
  @apply hidden;
}

.stocks-slider-outer .swiper-button-next,
.stocks-slider-outer .swiper-button-prev {
  @apply static! mt-0 h-8 w-9 rounded-full border dark:hover:bg-white/[0.05] border-gray-200 !text-gray-700 transition hover:bg-gray-100 dark:border-white/[0.03] dark:bg-gray-800 dark:!text-gray-400  dark:hover:!text-white/90;
}

.stocks-slider-outer .swiper-button-next.swiper-button-disabled,
.stocks-slider-outer .swiper-button-prev.swiper-button-disabled {
  @apply bg-white opacity-50 dark:bg-gray-900;
}

.stocks-slider-outer .swiper-button-next svg,
.stocks-slider-outer .swiper-button-prev svg {
  @apply !h-auto !w-auto;
}

.flatpickr-wrapper {
  @apply w-full;
}
.flatpickr-calendar {
  @apply mt-2 !bg-white !rounded-xl !p-5 !border !border-gray-200 !text-gray-500 dark:!bg-gray-dark dark:!border-gray-700 dark:!text-gray-400 dark:!shadow-theme-xl 2xsm:!w-auto;
}

/* Removed old hover stroke rule - now using orange theme colors in the main flatpickr styles */
.flatpickr-calendar.arrowTop:before,
.flatpickr-calendar.arrowTop:after {
  @apply hidden;
}

/* Removed old flatpickr-current-month rules - now using orange theme in main styles */

.flatpickr-prev-month,
.flatpickr-next-month {
  @apply !p-0;
}

.flatpickr-weekdays {
  @apply h-auto mt-6 mb-4 !bg-transparent;
}

.flatpickr-weekday {
  @apply !text-theme-sm !font-medium !text-gray-500 dark:!text-gray-400 !bg-transparent;
}

.flatpickr-day {
  @apply !flex !items-center !text-theme-sm !font-medium !text-gray-800 dark:!text-white/90 dark:hover:!border-gray-300 dark:hover:!bg-gray-900;
}
.flatpickr-day.nextMonthDay,
.flatpickr-day.prevMonthDay {
  @apply !text-gray-400 dark:!text-gray-600;
}

.flatpickr-months > .flatpickr-month {
  background: none !important;
}
/* Removed old flatpickr-monthDropdown-months rules - now using orange theme in main styles */
/* Removed old positioning rules - now using absolute positioning with center alignment */
.flatpickr-days {
  @apply !border-0;
}
span.flatpickr-weekday,
.flatpickr-months .flatpickr-month {
  @apply dark:!fill-white dark:!text-white !bg-none;
}
.flatpickr-innerContainer {
  @apply !border-b-0;
}
.flatpickr-months .flatpickr-month {
  @apply !bg-none;
}
.flatpickr-day.inRange {
  box-shadow: -5px 0 0 #f9fafb, 5px 0 0 #f9fafb !important;
  @apply dark:!shadow-datepicker;
}
.flatpickr-day.inRange,
.flatpickr-day.prevMonthDay.inRange,
.flatpickr-day.nextMonthDay.inRange,
.flatpickr-day.today.inRange,
.flatpickr-day.prevMonthDay.today.inRange,
.flatpickr-day.nextMonthDay.today.inRange,
.flatpickr-day:hover,
.flatpickr-day.prevMonthDay:hover,
.flatpickr-day.nextMonthDay:hover,
.flatpickr-day:focus,
.flatpickr-day.prevMonthDay:focus,
.flatpickr-day.nextMonthDay:focus {
  @apply !border-gray-50 !bg-gray-50 dark:!border-0 dark:!border-white/5 dark:!bg-white/5;
}
.flatpickr-day.selected,
.flatpickr-day.startRange,
.flatpickr-day.selected,
.flatpickr-day.endRange {
  @apply !text-white dark:!text-white;
}
.flatpickr-day.selected,
.flatpickr-day.startRange,
.flatpickr-day.endRange,
.flatpickr-day.selected.inRange,
.flatpickr-day.startRange.inRange,
.flatpickr-day.endRange.inRange,
.flatpickr-day.selected:focus,
.flatpickr-day.startRange:focus,
.flatpickr-day.endRange:focus,
.flatpickr-day.selected:hover,
.flatpickr-day.startRange:hover,
.flatpickr-day.endRange:hover,
.flatpickr-day.selected.prevMonthDay,
.flatpickr-day.startRange.prevMonthDay,
.flatpickr-day.endRange.prevMonthDay,
.flatpickr-day.selected.nextMonthDay,
.flatpickr-day.startRange.nextMonthDay,
.flatpickr-day.endRange.nextMonthDay {
  background: #fb6514;
  @apply !border-orange-500 !bg-orange-500 hover:!border-orange-500 hover:!bg-orange-500;
}
.flatpickr-day.selected.startRange + .endRange:not(:nth-child(7n + 1)),
.flatpickr-day.startRange.startRange + .endRange:not(:nth-child(7n + 1)),
.flatpickr-day.endRange.startRange + .endRange:not(:nth-child(7n + 1)) {
  box-shadow: -10px 0 0 #fb6514;
}

/* Removed old hover fill rules - now using orange theme colors */

.flatpickr-calendar.static {
  @apply right-0;
}

.fc .fc-view-harness {
  @apply max-w-full overflow-x-auto custom-scrollbar;
}

/* Custom animations for member cards */
@keyframes slideInFromBottom {
  from {
    opacity: 0;
    transform: translateY(20px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}

@keyframes fadeIn {
  from {
    opacity: 0;
  }
  to {
    opacity: 1;
  }
}

@keyframes zoomIn {
  from {
    opacity: 0;
    transform: scale(0.95);
  }
  to {
    opacity: 1;
    transform: scale(1);
  }
}

.animate-in {
  animation-fill-mode: both;
}

.fade-in {
  animation: fadeIn 0.3s ease-out;
}

.zoom-in-95 {
  animation: zoomIn 0.3s ease-out;
}

.slide-in-from-bottom-4 {
  animation: slideInFromBottom 0.5s ease-out;
}

/* Status-specific styling for class schedules */
.status-scheduled {
  @apply bg-orange-100 text-orange-800 border-orange-200;
}

.status-in-progress {
  @apply bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 border-green-200 dark:border-green-800;
}

.status-completed {
  @apply bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-200 dark:border-gray-700;
}

.status-cancelled {
  @apply bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 border-red-200 dark:border-red-800;
}

.status-postponed {
  @apply bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800;
}

/* Dark mode status styling */
.dark .status-scheduled {
  @apply bg-orange-900/30 text-orange-300 border-orange-700;
}

.dark .status-in-progress {
  @apply bg-green-900/30 text-green-300 border-green-700;
}

.dark .status-completed {
  @apply bg-gray-800/30 text-gray-300 border-gray-700;
}

.dark .status-cancelled {
  @apply bg-red-900/30 text-red-300 border-red-700;
}

.dark .status-postponed {
  @apply bg-yellow-900/30 text-yellow-300 border-yellow-700;
}
.fc-dayGridMonth-view.fc-view.fc-daygrid {
  @apply min-w-[718px];
}
.fc .fc-scrollgrid-section > * {
  border-right-width: 0;
  border-bottom-width: 0;
}
.fc .fc-scrollgrid {
  border-left-width: 0;
}
.fc .fc-toolbar.fc-header-toolbar {
  @apply flex-col gap-4 px-6 pt-6 sm:flex-row;
}
.fc-button-group {
  @apply gap-2;
}
.fc-button-group .fc-button {
  @apply flex h-10 w-10 items-center justify-center !rounded-lg border border-gray-200 bg-transparent hover:border-gray-200 hover:bg-gray-50 focus:shadow-none active:!border-gray-200 active:!bg-transparent active:!shadow-none dark:border-gray-800 dark:hover:border-gray-800 dark:hover:bg-gray-900 dark:active:border-gray-800!;
}

.fc-button-group .fc-button.fc-prev-button:before {
  @apply inline-block mt-1;
  content: url("data:image/svg+xml,%3Csvg width='25' height='24' viewBox='0 0 25 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M16.0068 6L9.75684 12.25L16.0068 18.5' stroke='%23344054' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E%0A");
}
.fc-button-group .fc-button.fc-next-button:before {
  @apply inline-block mt-1;
  content: url("data:image/svg+xml,%3Csvg width='25' height='24' viewBox='0 0 25 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M9.50684 19L15.7568 12.75L9.50684 6.5' stroke='%23344054' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E%0A");
}
.dark .fc-button-group .fc-button.fc-prev-button:before {
  content: url("data:image/svg+xml,%3Csvg width='25' height='24' viewBox='0 0 25 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M16.0068 6L9.75684 12.25L16.0068 18.5' stroke='%2398A2B3' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E%0A");
}
.dark .fc-button-group .fc-button.fc-next-button:before {
  content: url("data:image/svg+xml,%3Csvg width='25' height='24' viewBox='0 0 25 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M9.50684 19L15.7568 12.75L9.50684 6.5' stroke='%2398A2B3' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E%0A");
}
.fc-button-group .fc-button .fc-icon {
  @apply hidden;
}
.fc-addEventButton-button {
  @apply !rounded-lg !border-0 !bg-brand-500 !px-4 !py-2.5 !text-sm !font-medium hover:!bg-brand-600 focus:!shadow-none;
}
.fc-toolbar-title {
  @apply !text-lg !font-medium text-gray-800 dark:text-white/90;
}
.fc-header-toolbar.fc-toolbar .fc-toolbar-chunk:last-child {
  @apply rounded-lg bg-gray-100 p-0.5 dark:bg-gray-900;
}
.fc-header-toolbar.fc-toolbar .fc-toolbar-chunk:last-child .fc-button {
  @apply !h-auto !w-auto rounded-md !border-0 bg-transparent !px-5 !py-2 text-sm font-medium text-gray-500 hover:text-gray-700 focus:!shadow-none dark:text-gray-400;
}
.fc-header-toolbar.fc-toolbar .fc-toolbar-chunk:last-child .fc-button.fc-button-active {
  @apply text-gray-900 bg-white dark:bg-gray-800 dark:text-white;
}
.fc-theme-standard th {
  @apply !border-x-0 border-t !border-gray-200 bg-gray-50 !text-left dark:!border-gray-800 dark:bg-gray-900;
}
.fc-theme-standard td,
.fc-theme-standard .fc-scrollgrid {
  @apply !border-gray-200 dark:!border-gray-800;
}
.fc .fc-col-header-cell-cushion {
  @apply !px-5 !py-4 text-sm font-medium uppercase text-gray-400 dark:text-gray-500;
}
.fc .fc-daygrid-day.fc-day-today {
  @apply bg-transparent;
}
.fc .fc-daygrid-day {
  @apply p-2;
}
.fc .fc-daygrid-day.fc-day-today .fc-scrollgrid-sync-inner {
  @apply rounded-sm bg-gray-100 dark:bg-white/[0.03];
}
.fc .fc-daygrid-day-number {
  @apply !p-3 text-sm font-medium text-gray-700 dark:text-gray-400;
}
.fc .fc-daygrid-day-top {
  @apply !flex-row;
}
.fc .fc-day-other .fc-daygrid-day-top {
  opacity: 1;
}
.fc .fc-day-other .fc-daygrid-day-top .fc-daygrid-day-number {
  @apply text-gray-400 dark:text-white/30;
}
.event-fc-color {
  @apply rounded-lg py-2.5 pl-4 pr-3;
}
.event-fc-color .fc-event-title {
  @apply p-0 text-sm font-normal text-gray-700 dark:text-gray-200;
}
.fc-daygrid-event-dot {
  @apply w-1 h-5 ml-0 mr-3 border-none rounded-sm;
}
.fc-event {
  @apply focus:shadow-none;
}
.fc-daygrid-event.fc-event-start {
  @apply !ml-3;
}
.event-fc-color.fc-bg-success {
  @apply border-success-50 bg-success-50 dark:border-success-900/30 dark:bg-success-900/20;
}
.event-fc-color.fc-bg-danger {
  @apply border-error-50 bg-error-50 dark:border-error-900/30 dark:bg-error-900/20;
}
.event-fc-color.fc-bg-primary {
  @apply border-brand-50 bg-brand-50 dark:border-orange-900/30 dark:bg-orange-900/20;
}
.event-fc-color.fc-bg-warning {
  @apply border-orange-50 bg-orange-50 dark:border-orange-900/30 dark:bg-orange-900/20;
}
.event-fc-color.fc-bg-success .fc-daygrid-event-dot {
  @apply bg-success-500;
}
.event-fc-color.fc-bg-danger .fc-daygrid-event-dot {
  @apply bg-error-500;
}
.event-fc-color.fc-bg-primary .fc-daygrid-event-dot {
  @apply bg-brand-500;
}
.event-fc-color.fc-bg-warning .fc-daygrid-event-dot {
  @apply bg-orange-500;
}
.fc-direction-ltr .fc-timegrid-slot-label-frame {
  @apply px-3 py-1.5 text-left text-sm font-medium text-gray-500 dark:text-gray-400;
}
.fc .fc-timegrid-axis-cushion {
  @apply text-sm font-medium text-gray-500 dark:text-gray-400;
}

.input-date-icon::-webkit-inner-spin-button,
.input-date-icon::-webkit-calendar-picker-indicator {
  opacity: 0;
  -webkit-appearance: none;
}

/* Header search input - prevent CSS override */
.header-search-input {
  height: 30px !important;
  font-size: 11px !important;
  padding-top: 0 !important;
  padding-bottom: 0 !important;
  line-height: 30px !important;
}

/* Header search input - prevent CSS override with maximum specificity */
/* Override ALL possible blue/brand colors */
header .header-search-input,
header form .header-search-input,
header form input.header-search-input,
header form input[type='text'].header-search-input {
  border-color: rgb(209, 213, 219) !important; /* gray-300 */
}

header .header-search-input:focus,
header .header-search-input:focus-visible,
header .header-search-input:active,
header form .header-search-input:focus,
header form .header-search-input:focus-visible,
header form input.header-search-input:focus,
header form input.header-search-input:focus-visible,
header form input[type='text'].header-search-input:focus,
header form input[type='text'].header-search-input:focus-visible {
  border-color: #f97316 !important; /* orange-500 - override any blue */
  outline: none !important;
  box-shadow: 0 0 0 2px rgba(249, 115, 22, 0.3) !important; /* orange-500/30 */
}

/* Override brand colors specifically - highest priority */
header .header-search-input:focus[class*='brand'],
header form .header-search-input:focus[class*='brand'],
header form input.header-search-input:focus[class*='brand'],
header form input[type='text'].header-search-input:focus[class*='brand'] {
  border-color: #f97316 !important;
  box-shadow: 0 0 0 2px rgba(249, 115, 22, 0.3) !important;
}

.dark header .header-search-input:focus,
.dark header .header-search-input:focus-visible,
.dark header .header-search-input:active,
.dark header form .header-search-input:focus,
.dark header form .header-search-input:focus-visible,
.dark header form input.header-search-input:focus,
.dark header form input.header-search-input:focus-visible,
.dark header form input[type='text'].header-search-input:focus,
.dark header form input[type='text'].header-search-input:focus-visible {
  border-color: #f97316 !important; /* orange-500 */
  box-shadow: 0 0 0 2px rgba(249, 115, 22, 0.3) !important; /* orange-500/30 */
}

.swiper-button-prev svg,
.swiper-button-next svg {
  @apply !h-auto !w-auto;
}

.carouselTwo .swiper-button-next:after,
.carouselTwo .swiper-button-prev:after,
.carouselFour .swiper-button-next:after,
.carouselFour .swiper-button-prev:after {
  @apply hidden;
}
.carouselTwo .swiper-button-next.swiper-button-disabled,
.carouselTwo .swiper-button-prev.swiper-button-disabled,
.carouselFour .swiper-button-next.swiper-button-disabled,
.carouselFour .swiper-button-prev.swiper-button-disabled {
  @apply bg-white/60 !opacity-100;
}
.carouselTwo .swiper-button-next,
.carouselTwo .swiper-button-prev,
.carouselFour .swiper-button-next,
.carouselFour .swiper-button-prev {
  @apply h-10 w-10 rounded-full border-[0.5px] border-white/10 bg-white/90 !text-gray-700 shadow-slider-navigation backdrop-blur-[10px];
}

.carouselTwo .swiper-button-prev,
.carouselFour .swiper-button-prev {
  @apply !left-3 sm:!left-4;
}

.carouselTwo .swiper-button-next,
.carouselFour .swiper-button-next {
  @apply !right-3 sm:!right-4;
}

.carouselThree .swiper-pagination,
.carouselFour .swiper-pagination {
  @apply !bottom-3 !left-1/2 inline-flex !w-auto -translate-x-1/2 items-center gap-1.5 rounded-[40px] border-[0.5px] border-white/10 bg-white/60 px-2 py-1.5 shadow-slider-navigation backdrop-blur-[10px] sm:!bottom-5;
}

.carouselThree .swiper-pagination-bullet,
.carouselFour .swiper-pagination-bullet {
  @apply !m-0 h-2.5 w-2.5 bg-white opacity-100 shadow-theme-xs duration-200 ease-in-out;
}

.carouselThree .swiper-pagination-bullet-active,
.carouselFour .swiper-pagination-bullet-active {
  @apply w-6.5 rounded-xl;
}

.form-check-input:checked ~ span {
  @apply border-[6px] border-brand-500 bg-brand-500 dark:border-brand-500;
}

.taskCheckbox:checked ~ .box span {
  @apply opacity-100 bg-brand-500;
}
.taskCheckbox:checked ~ p {
  @apply text-gray-400 line-through;
}
.taskCheckbox:checked ~ .box {
  @apply border-brand-500 bg-brand-500 dark:border-brand-500;
}

.task {
  transition: all 0.2s ease; /* Smooth transition for visual effects */
}

.task {
  border-radius: 0.75rem;
  box-shadow: 0px 1px 3px 0px rgba(16, 24, 40, 0.1), 0px 1px 2px 0px rgba(16, 24, 40, 0.06);
  opacity: 0.8;
  cursor: grabbing; /* Changes the cursor to indicate dragging */
}

.custom-calendar .fc-h-event {
  background-color: transparent;
  border: none;
  color: black;
}
.fc.fc-media-screen {
  @apply min-h-screen;
}

.simplebar-scrollbar::before {
  @apply !bg-gray-200 !rounded-full dark:!bg-gray-700 !opacity-100;
}

.dark .simplebar-scrollbar::before {
  @apply !bg-gray-700;
}

.simplebar-scrollbar.simplebar-visible:before {
  @apply opacity-100;
}

/* Enhanced Auth Form Animations */
@keyframes inputFocus {
  0% {
    transform: scale(1);
  }
  50% {
    transform: scale(1.02);
  }
  100% {
    transform: scale(1);
  }
}

@keyframes buttonPulse {
  0% {
    box-shadow: 0 0 0 0 rgba(243, 97, 0, 0.4);
  }
  70% {
    box-shadow: 0 0 0 10px rgba(243, 97, 0, 0);
  }
  100% {
    box-shadow: 0 0 0 0 rgba(243, 97, 0, 0);
  }
}

@keyframes iconBounce {
  0%,
  20%,
  50%,
  80%,
  100% {
    transform: translateY(0);
  }
  40% {
    transform: translateY(-3px);
  }
  60% {
    transform: translateY(-2px);
  }
}

@keyframes formSlideIn {
  0% {
    opacity: 0;
    transform: translateY(30px) scale(0.95);
  }
  100% {
    opacity: 1;
    transform: translateY(0) scale(1);
  }
}

/* Auth form enhanced styling */
.auth-form-container {
  animation: formSlideIn 0.6s ease-out;
}

.auth-input:focus {
  animation: inputFocus 0.3s ease-out;
}

.auth-button:hover {
  animation: buttonPulse 1.5s infinite;
}

.auth-icon {
  animation: iconBounce 2s infinite;
}

.auth-icon:hover {
  animation: iconBounce 0.6s ease-in-out;
}

/* Toggle button animations */
.auth-toggle-active {
  background: linear-gradient(135deg, #f36100 0%, #e55a2b 100%);
  box-shadow: 0 4px 15px rgba(243, 97, 0, 0.3);
  transform: scale(1.05);
}

.auth-toggle-inactive {
  background: rgba(255, 255, 255, 0.1);
  transition: all 0.3s ease;
}

.auth-toggle-inactive:hover {
  background: rgba(255, 255, 255, 0.15);
  transform: scale(1.02);
}

/* Title Animation */
@keyframes fadeInUp {
  from {
    opacity: 0;
    transform: translateY(20px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}

.animate-fade-in-up {
  animation: fadeInUp 0.8s ease-out forwards;
  opacity: 0;
}

.delay-200 {
  animation-delay: 0.2s;
}

/* Form Slide Animation */
@keyframes slideInFromRight {
  from {
    opacity: 0;
    transform: translateX(100px);
  }
  to {
    opacity: 1;
    transform: translateX(0);
  }
}

@keyframes slideInFromLeft {
  from {
    opacity: 0;
    transform: translateX(-100px);
  }
  to {
    opacity: 1;
    transform: translateX(0);
  }
}

@keyframes slideOutToRight {
  from {
    opacity: 1;
    transform: translateX(0);
  }
  to {
    opacity: 0;
    transform: translateX(100px);
  }
}

@keyframes slideOutToLeft {
  from {
    opacity: 1;
    transform: translateX(0);
  }
  to {
    opacity: 0;
    transform: translateX(-100px);
  }
}

.animate-slide-in-right {
  animation: slideInFromRight 0.6s ease-out forwards;
}

.animate-slide-in-left {
  animation: slideInFromLeft 0.6s ease-out forwards;
}

.animate-slide-out-right {
  animation: slideOutToRight 0.4s ease-in forwards;
}

.animate-slide-out-left {
  animation: slideOutToLeft 0.4s ease-in forwards;
}

/* Custom animations for forgot password title */
@keyframes slideInFromTop {
  0% {
    transform: translateY(-30px);
    opacity: 0;
  }
  100% {
    transform: translateY(0);
    opacity: 1;
  }
}

@keyframes glow {
  0% {
    text-shadow: 0 0 10px rgba(255, 255, 255, 0.5), 0 0 20px rgba(255, 255, 255, 0.3);
  }
  100% {
    text-shadow: 0 0 20px rgba(255, 255, 255, 0.8), 0 0 30px rgba(255, 255, 255, 0.5),
      0 0 40px rgba(255, 255, 255, 0.3);
  }
}

/* Enhanced bounce animation with different delays */
.delay-100 {
  animation-delay: 0.1s;
}
.delay-200 {
  animation-delay: 0.2s;
}
.delay-300 {
  animation-delay: 0.3s;
}
.delay-400 {
  animation-delay: 0.4s;
}
.delay-500 {
  animation-delay: 0.5s;
}
.delay-600 {
  animation-delay: 0.6s;
}
.delay-700 {
  animation-delay: 0.7s;
}
.delay-800 {
  animation-delay: 0.8s;
}
.delay-900 {
  animation-delay: 0.9s;
}
.delay-1000 {
  animation-delay: 1s;
}
.delay-1100 {
  animation-delay: 1.1s;
}

/* Custom Select Scrollbar - Smaller and more subtle */
.custom-select-scrollbar {
  scrollbar-width: thin;
  scrollbar-color: #d1d5db rgba(0, 0, 0, 0.05);
}

.custom-select-scrollbar::-webkit-scrollbar {
  width: 4px;
  height: 4px;
}

.custom-select-scrollbar::-webkit-scrollbar-track {
  background: rgba(0, 0, 0, 0.02);
  border-radius: 4px;
}

.custom-select-scrollbar::-webkit-scrollbar-thumb {
  background: #d1d5db;
  border-radius: 4px;
  transition: background 0.2s ease;
}

.custom-select-scrollbar::-webkit-scrollbar-thumb:hover {
  background: #9ca3af;
}

.dark .custom-select-scrollbar {
  scrollbar-color: #4b5563 rgba(255, 255, 255, 0.05);
}

.dark .custom-select-scrollbar::-webkit-scrollbar-track {
  background: rgba(255, 255, 255, 0.02);
}

.dark .custom-select-scrollbar::-webkit-scrollbar-thumb {
  background: #4b5563;
}

.dark .custom-select-scrollbar::-webkit-scrollbar-thumb:hover {
  background: #6b7280;
}

/* Hide scrollbar but allow scrolling */
.scrollbar-hide {
  /* Chrome, Safari and Opera */
  &::-webkit-scrollbar {
    display: none;
  }
  /* IE and Edge */
  -ms-overflow-style: none;
  /* Firefox */
  scrollbar-width: none;
}

/* Modal Scrollbar - Extra small and subtle */
.modal-scrollbar {
  scrollbar-width: thin;
  scrollbar-color: #d1d5db transparent;
}

.modal-scrollbar::-webkit-scrollbar {
  width: 3px;
  height: 3px;
}

.modal-scrollbar::-webkit-scrollbar-track {
  background: transparent;
  border-radius: 3px;
}

.modal-scrollbar::-webkit-scrollbar-thumb {
  background-color: #d1d5db;
  border-radius: 3px;
  transition: background-color 0.2s ease;
}

.modal-scrollbar::-webkit-scrollbar-thumb:hover {
  background-color: #9ca3af;
}

.dark .modal-scrollbar {
  scrollbar-color: #4b5563 transparent;
}

.dark .modal-scrollbar::-webkit-scrollbar-track {
  background: transparent;
}

.dark .modal-scrollbar::-webkit-scrollbar-thumb {
  background-color: #4b5563;
}

.dark .modal-scrollbar::-webkit-scrollbar-thumb:hover {
  background-color: #6b7280;
}

/* Flatpickr Calendar - Orange Theme */
.flatpickr-calendar {
  background: var(--color-white) !important;
  border: 1px solid var(--color-orange-300) !important;
  border-radius: 0.5rem !important;
  box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05) !important;
  font-family: 'Space Grotesk', sans-serif !important;
  padding: 0.5rem !important; /* Original size */
}

.dark .flatpickr-calendar {
  background: var(--color-gray-800) !important;
  border-color: var(--color-orange-700) !important;
  box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.3), 0 4px 6px -2px rgba(0, 0, 0, 0.2) !important;
}

/* Month/Year Navigation */
.flatpickr-months {
  background: var(--color-orange-50) !important;
  border-radius: 0.5rem 0.5rem 0 0 !important; /* Original size */
  padding: 0.5rem 0 !important; /* Original size */
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  position: relative !important;
}

.dark .flatpickr-months {
  background: var(--color-orange-900) !important;
}

.flatpickr-month {
  color: var(--color-gray-900) !important;
  height: 2.5rem !important; /* Original size */
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  position: relative !important;
}

.dark .flatpickr-month {
  color: var(--color-white) !important;
}

.flatpickr-current-month {
  font-size: 0.875rem !important; /* Original size */
  font-weight: 600 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  padding: 0.5rem 0 !important; /* Original size */
}

/* Month and Year Select Dropdowns - Orange Theme */
.flatpickr-current-month .cur-month,
.flatpickr-current-month input.cur-year {
  color: var(--color-orange-700) !important;
  font-size: 0.875rem !important;
  font-weight: 600 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  background: transparent !important;
  border: 1px solid transparent !important;
  border-radius: 0.375rem !important;
  padding: 0.25rem 0.5rem !important;
  cursor: pointer !important;
  transition: all 0.2s ease !important;
  appearance: none !important;
  -webkit-appearance: none !important;
  -moz-appearance: none !important;
}

.flatpickr-current-month .cur-month:hover,
.flatpickr-current-month input.cur-year:hover {
  background: var(--color-orange-100) !important;
  border-color: var(--color-orange-300) !important;
  color: var(--color-orange-800) !important;
}

.flatpickr-current-month .cur-month:focus,
.flatpickr-current-month input.cur-year:focus {
  outline: none !important;
  background: var(--color-orange-50) !important;
  border-color: var(--color-orange-400) !important;
  color: var(--color-orange-800) !important;
  box-shadow: 0 0 0 2px rgba(249, 115, 22, 0.2) !important;
}

.dark .flatpickr-current-month .cur-month,
.dark .flatpickr-current-month input.cur-year {
  color: var(--color-orange-300) !important;
}

.dark .flatpickr-current-month .cur-month:hover,
.dark .flatpickr-current-month input.cur-year:hover {
  background: var(--color-orange-800) !important;
  border-color: var(--color-orange-600) !important;
  color: var(--color-orange-200) !important;
}

.dark .flatpickr-current-month .cur-month:focus,
.dark .flatpickr-current-month input.cur-year:focus {
  background: var(--color-orange-900) !important;
  border-color: var(--color-orange-500) !important;
  color: var(--color-orange-200) !important;
  box-shadow: 0 0 0 2px rgba(249, 115, 22, 0.3) !important;
}

/* Month Dropdown Select */
.flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months {
  color: var(--color-orange-700) !important;
  font-size: 0.875rem !important;
  font-weight: 600 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  background: transparent !important;
  border: 1px solid transparent !important;
  border-radius: 0.375rem !important;
  padding: 0.25rem 0.5rem !important;
  cursor: pointer !important;
  transition: all 0.2s ease !important;
  appearance: none !important;
  -webkit-appearance: none !important;
  -moz-appearance: none !important;
  background-image: none !important;
}

.flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months:hover {
  background: var(--color-orange-100) !important;
  border-color: var(--color-orange-300) !important;
  color: var(--color-orange-800) !important;
}

.flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months:focus {
  outline: none !important;
  background: var(--color-orange-50) !important;
  border-color: var(--color-orange-400) !important;
  color: var(--color-orange-800) !important;
  box-shadow: 0 0 0 2px rgba(249, 115, 22, 0.2) !important;
}

.dark .flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months {
  color: var(--color-orange-300) !important;
}

.dark .flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months:hover {
  background: var(--color-orange-800) !important;
  border-color: var(--color-orange-600) !important;
  color: var(--color-orange-200) !important;
}

.dark .flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months:focus {
  background: var(--color-orange-900) !important;
  border-color: var(--color-orange-500) !important;
  color: var(--color-orange-200) !important;
  box-shadow: 0 0 0 2px rgba(249, 115, 22, 0.3) !important;
}

/* Dropdown Option Items - Orange Theme */
.flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option {
  background: var(--color-white) !important;
  color: var(--color-orange-700) !important;
  font-size: 0.875rem !important;
  font-weight: 600 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  padding: 0.5rem !important;
  border: none !important;
}

.flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option:hover {
  background: var(--color-orange-100) !important;
  color: var(--color-orange-800) !important;
}

.flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option:checked,
.flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option:focus {
  background: var(--color-orange-500) !important;
  color: var(--color-white) !important;
}

.dark .flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option {
  background: var(--color-gray-800) !important;
  color: var(--color-orange-300) !important;
}

.dark .flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option:hover {
  background: var(--color-orange-800) !important;
  color: var(--color-orange-200) !important;
}

.dark .flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option:checked,
.dark .flatpickr-month .flatpickr-current-month .flatpickr-monthDropdown-months option:focus {
  background: var(--color-orange-600) !important;
  color: var(--color-white) !important;
}

/* Year Input - Orange Theme (already covered in the combined selector above, but adding specific width) */
.flatpickr-current-month input.cur-year,
.flatpickr-current-month input[type='number'].cur-year,
.flatpickr-month .flatpickr-current-month input.cur-year,
.flatpickr-calendar input[type='number'],
.flatpickr-calendar .flatpickr-current-month input[type='number'] {
  width: 3.5rem !important;
  min-width: 3.5rem !important;
  max-width: 3.5rem !important;
  cursor: text !important;
  /* Hide number input spinner buttons - Force hide */
  appearance: textfield !important;
  -moz-appearance: textfield !important;
  -webkit-appearance: textfield !important;
}

/* Ensure numInputWrapper doesn't affect input width */
.flatpickr-calendar .numInputWrapper,
.flatpickr-current-month .numInputWrapper,
.flatpickr-month .numInputWrapper {
  display: inline-block !important;
  width: auto !important;
}

.flatpickr-calendar .numInputWrapper input.cur-year,
.flatpickr-current-month .numInputWrapper input.cur-year,
.flatpickr-month .numInputWrapper input.cur-year {
  width: 3.5rem !important;
  min-width: 3.5rem !important;
  max-width: 3.5rem !important;
}

/* Hide spinner buttons for year input in WebKit browsers - Multiple selectors for maximum compatibility */
.flatpickr-current-month input.cur-year::-webkit-inner-spin-button,
.flatpickr-current-month input.cur-year::-webkit-outer-spin-button,
.flatpickr-current-month input[type='number'].cur-year::-webkit-inner-spin-button,
.flatpickr-current-month input[type='number'].cur-year::-webkit-outer-spin-button,
.flatpickr-month .flatpickr-current-month input.cur-year::-webkit-inner-spin-button,
.flatpickr-month .flatpickr-current-month input.cur-year::-webkit-outer-spin-button,
.flatpickr-month .flatpickr-current-month input[type='number'].cur-year::-webkit-inner-spin-button,
.flatpickr-month .flatpickr-current-month input[type='number'].cur-year::-webkit-outer-spin-button,
/* Hide on hover and focus states */
.flatpickr-current-month input.cur-year:hover::-webkit-inner-spin-button,
.flatpickr-current-month input.cur-year:hover::-webkit-outer-spin-button,
.flatpickr-current-month input.cur-year:focus::-webkit-inner-spin-button,
.flatpickr-current-month input.cur-year:focus::-webkit-outer-spin-button,
.flatpickr-current-month input[type='number'].cur-year:hover::-webkit-inner-spin-button,
.flatpickr-current-month input[type='number'].cur-year:hover::-webkit-outer-spin-button,
.flatpickr-current-month input[type='number'].cur-year:focus::-webkit-inner-spin-button,
.flatpickr-current-month input[type='number'].cur-year:focus::-webkit-outer-spin-button,
.flatpickr-month .flatpickr-current-month input.cur-year:hover::-webkit-inner-spin-button,
.flatpickr-month .flatpickr-current-month input.cur-year:hover::-webkit-outer-spin-button,
.flatpickr-month .flatpickr-current-month input.cur-year:focus::-webkit-inner-spin-button,
.flatpickr-month .flatpickr-current-month input.cur-year:focus::-webkit-outer-spin-button,
.flatpickr-month .flatpickr-current-month input[type='number'].cur-year:hover::-webkit-inner-spin-button,
.flatpickr-month .flatpickr-current-month input[type='number'].cur-year:hover::-webkit-outer-spin-button,
.flatpickr-month .flatpickr-current-month input[type='number'].cur-year:focus::-webkit-inner-spin-button,
.flatpickr-month .flatpickr-current-month input[type='number'].cur-year:focus::-webkit-outer-spin-button {
  -webkit-appearance: none !important;
  appearance: none !important;
  margin: 0 !important;
  display: none !important;
  opacity: 0 !important;
  visibility: hidden !important;
  pointer-events: none !important;
  width: 0 !important;
  height: 0 !important;
  position: absolute !important;
  left: -9999px !important;
}

/* Global hide for all number inputs in flatpickr - Additional coverage */
.flatpickr-calendar input[type='number']::-webkit-inner-spin-button,
.flatpickr-calendar input[type='number']::-webkit-outer-spin-button,
.flatpickr-calendar input[type='number']:hover::-webkit-inner-spin-button,
.flatpickr-calendar input[type='number']:hover::-webkit-outer-spin-button,
.flatpickr-calendar input[type='number']:focus::-webkit-inner-spin-button,
.flatpickr-calendar input[type='number']:focus::-webkit-outer-spin-button,
.flatpickr-calendar .flatpickr-current-month input[type='number']::-webkit-inner-spin-button,
.flatpickr-calendar .flatpickr-current-month input[type='number']::-webkit-outer-spin-button,
.flatpickr-calendar .flatpickr-current-month input[type='number']:hover::-webkit-inner-spin-button,
.flatpickr-calendar .flatpickr-current-month input[type='number']:hover::-webkit-outer-spin-button,
.flatpickr-calendar .flatpickr-current-month input[type='number']:focus::-webkit-inner-spin-button,
.flatpickr-calendar .flatpickr-current-month input[type='number']:focus::-webkit-outer-spin-button {
  -webkit-appearance: none !important;
  appearance: none !important;
  margin: 0 !important;
  display: none !important;
  opacity: 0 !important;
  visibility: hidden !important;
  pointer-events: none !important;
  width: 0 !important;
  height: 0 !important;
  position: absolute !important;
  left: -9999px !important;
}

/* Hide numInputWrapper arrow buttons only, keep the input visible */
.flatpickr-calendar .numInputWrapper .arrowUp,
.flatpickr-calendar .numInputWrapper .arrowDown,
.flatpickr-current-month .numInputWrapper .arrowUp,
.flatpickr-current-month .numInputWrapper .arrowDown,
.flatpickr-month .numInputWrapper .arrowUp,
.flatpickr-month .numInputWrapper .arrowDown {
  display: none !important;
  visibility: hidden !important;
  opacity: 0 !important;
  pointer-events: none !important;
  width: 0 !important;
  height: 0 !important;
  position: absolute !important;
  left: -9999px !important;
}

/* Hide arrowUp and arrowDown buttons in flatpickr calendar (but not the input itself) */
.flatpickr-calendar .arrowUp:not(input),
.flatpickr-calendar .arrowDown:not(input),
.flatpickr-current-month .arrowUp:not(input),
.flatpickr-current-month .arrowDown:not(input),
.flatpickr-month .arrowUp:not(input),
.flatpickr-month .arrowDown:not(input) {
  display: none !important;
  visibility: hidden !important;
  opacity: 0 !important;
  pointer-events: none !important;
  width: 0 !important;
  height: 0 !important;
  position: absolute !important;
  left: -9999px !important;
}

.flatpickr-prev-month,
.flatpickr-next-month {
  color: var(--color-orange-600) !important;
  fill: var(--color-orange-600) !important;
  padding: 0.5rem !important; /* Original size */
  border-radius: 0.375rem !important; /* Original size */
  transition: all 0.2s ease !important;
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  position: absolute !important;
  top: 50% !important;
  transform: translateY(-50%) !important;
  background: transparent !important;
}

.flatpickr-prev-month {
  left: 0.5rem !important;
}

.flatpickr-next-month {
  right: 0.5rem !important;
}

.flatpickr-prev-month:hover,
.flatpickr-next-month:hover {
  background: var(--color-orange-100) !important;
  color: var(--color-orange-700) !important;
  fill: var(--color-orange-700) !important;
}

.flatpickr-prev-month:hover svg,
.flatpickr-next-month:hover svg {
  fill: var(--color-orange-700) !important;
  stroke: var(--color-orange-700) !important;
}

.dark .flatpickr-prev-month,
.dark .flatpickr-next-month {
  color: var(--color-orange-400) !important;
  fill: var(--color-orange-400) !important;
}

.dark .flatpickr-prev-month svg,
.dark .flatpickr-next-month svg {
  fill: var(--color-orange-400) !important;
  stroke: var(--color-orange-400) !important;
}

.dark .flatpickr-prev-month:hover,
.dark .flatpickr-next-month:hover {
  background: var(--color-orange-800) !important;
  color: var(--color-orange-300) !important;
  fill: var(--color-orange-300) !important;
}

.dark .flatpickr-prev-month:hover svg,
.dark .flatpickr-next-month:hover svg {
  fill: var(--color-orange-300) !important;
  stroke: var(--color-orange-300) !important;
}

/* Weekday Headers */
.flatpickr-weekdays {
  background: transparent !important;
  border-bottom: 1px solid var(--color-gray-200) !important;
  padding: 0.5rem 0 !important; /* Original size */
}

.dark .flatpickr-weekdays {
  border-bottom-color: var(--color-gray-700) !important;
}

.flatpickr-weekday {
  color: var(--color-gray-600) !important;
  font-size: 0.75rem !important; /* Original size */
  font-weight: 600 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  text-transform: uppercase !important;
}

.dark .flatpickr-weekday {
  color: var(--color-gray-400) !important;
}

.flatpickr-day {
  color: var(--color-gray-900) !important;
  border-radius: 0.1875rem !important; /* 50% of 0.375rem */
  border: 1px solid transparent !important;
  font-size: 0.75rem !important; /* Original size */
  font-weight: 500 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  height: 2rem !important; /* Original size */
  line-height: 2rem !important; /* Original size */
  transition: all 0.2s ease !important;
}

.dark .flatpickr-day {
  color: var(--color-white) !important;
}

.flatpickr-day:hover {
  background: var(--color-orange-100) !important;
  border-color: var(--color-orange-300) !important;
  color: var(--color-orange-700) !important;
}

.dark .flatpickr-day:hover {
  background: var(--color-orange-900) !important;
  border-color: var(--color-orange-700) !important;
  color: var(--color-orange-300) !important;
}

/* Selected Date */
.flatpickr-day.selected {
  background: var(--color-orange-500) !important;
  border-color: var(--color-orange-500) !important;
  color: var(--color-white) !important;
  font-weight: 600 !important;
}

.flatpickr-day.selected:hover {
  background: var(--color-orange-600) !important;
  border-color: var(--color-orange-600) !important;
}

.flatpickr-day.selected.startRange,
.flatpickr-day.selected.endRange {
  background: var(--color-orange-500) !important;
  border-color: var(--color-orange-500) !important;
  color: var(--color-white) !important;
}

/* Range Selection */
.flatpickr-day.inRange {
  background: var(--color-orange-100) !important;
  border-color: var(--color-orange-200) !important;
  color: var(--color-orange-700) !important;
  box-shadow: -2px 0 0 var(--color-orange-100), 2px 0 0 var(--color-orange-100) !important;
}

.dark .flatpickr-day.inRange {
  background: var(--color-orange-900) !important;
  border-color: var(--color-orange-800) !important;
  color: var(--color-orange-300) !important;
  box-shadow: -2px 0 0 var(--color-orange-900), 2px 0 0 var(--color-orange-900) !important;
}

/* Today Indicator */
.flatpickr-day.today {
  border-color: var(--color-orange-500) !important;
  font-weight: 600 !important;
}

.flatpickr-day.today:hover {
  border-color: var(--color-orange-600) !important;
}

.flatpickr-day.today.selected {
  border-color: var(--color-orange-600) !important;
}

/* Disabled Dates */
.flatpickr-day.flatpickr-disabled,
.flatpickr-day.prevMonthDay,
.flatpickr-day.nextMonthDay {
  color: var(--color-gray-400) !important;
  opacity: 0.4 !important;
  cursor: not-allowed !important;
}

.dark .flatpickr-day.flatpickr-disabled,
.dark .flatpickr-day.prevMonthDay,
.dark .flatpickr-day.nextMonthDay {
  color: var(--color-gray-600) !important;
}

.flatpickr-day.flatpickr-disabled:hover,
.flatpickr-day.prevMonthDay:hover,
.flatpickr-day.nextMonthDay:hover {
  background: transparent !important;
  border-color: transparent !important;
}

/* Time Picker - Compact Professional Orange Theme */
.flatpickr-time {
  border-top: 1px solid var(--color-orange-200) !important;
  padding: 0.5rem 0.375rem !important; /* Original size */
  background: var(--color-orange-50) !important;
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  gap: 0.25rem !important; /* Original size */
}

.dark .flatpickr-time {
  border-top-color: var(--color-orange-800) !important;
  background: var(--color-orange-950) !important;
}

.flatpickr-time .flatpickr-time-wrapper {
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  gap: 0.25rem !important; /* Original size */
}

.flatpickr-time input {
  color: var(--color-gray-900) !important;
  font-size: 0.875rem !important; /* Original size */
  font-weight: 600 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  width: 2rem !important; /* Original size */
  height: 1.75rem !important; /* Original size */
  line-height: 1.75rem !important; /* Original size */
  padding: 0 !important;
  text-align: center !important;
  border: 1px solid var(--color-gray-300) !important;
  border-radius: 0.375rem !important; /* Original size */
  background: var(--color-white) !important;
  transition: all 0.2s ease !important;
  box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05) !important;
}

.dark .flatpickr-time input {
  color: var(--color-white) !important;
  background: var(--color-gray-800) !important;
  border-color: var(--color-gray-600) !important;
}

.flatpickr-time input:hover {
  background: var(--color-orange-50) !important;
  border-color: var(--color-orange-400) !important;
  color: var(--color-orange-700) !important;
  box-shadow: 0 2px 4px rgba(249, 115, 22, 0.15) !important;
  transform: translateY(-1px) !important;
}

.dark .flatpickr-time input:hover {
  background: var(--color-orange-900) !important;
  border-color: var(--color-orange-600) !important;
  color: var(--color-orange-300) !important;
}

.flatpickr-time input:focus {
  background: var(--color-orange-100) !important;
  border-color: var(--color-orange-500) !important;
  color: var(--color-orange-700) !important;
  box-shadow: 0 0 0 3px rgba(249, 115, 22, 0.2) !important;
  outline: none !important;
}

.dark .flatpickr-time input:focus {
  background: var(--color-orange-900) !important;
  border-color: var(--color-orange-500) !important;
  color: var(--color-orange-300) !important;
  box-shadow: 0 0 0 3px rgba(249, 115, 22, 0.3) !important;
}

.flatpickr-time .flatpickr-time-separator {
  color: var(--color-orange-600) !important;
  font-size: 0.875rem !important; /* Original size */
  font-weight: 700 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  margin: 0 0.125rem !important; /* Original size */
  padding: 0 !important;
}

.dark .flatpickr-time .flatpickr-time-separator {
  color: var(--color-orange-400) !important;
}

.flatpickr-time .arrowUp,
.flatpickr-time .arrowDown {
  display: none !important; /* Hide default arrows for cleaner look */
}

.flatpickr-am-pm {
  color: var(--color-gray-700) !important;
  font-size: 0.75rem !important; /* Original size */
  font-weight: 600 !important;
  font-family: 'Space Grotesk', sans-serif !important;
  padding: 0.25rem 0.5rem !important; /* Original size */
  border: 1px solid var(--color-gray-300) !important;
  border-radius: 0.375rem !important; /* Original size */
  background: var(--color-white) !important;
  transition: all 0.2s ease !important;
  cursor: pointer !important;
  min-width: 2.5rem !important; /* Original size */
  text-align: center !important;
}

.dark .flatpickr-am-pm {
  color: var(--color-gray-300) !important;
  background: var(--color-gray-800) !important;
  border-color: var(--color-gray-600) !important;
}

.flatpickr-am-pm:hover {
  background: var(--color-orange-100) !important;
  border-color: var(--color-orange-400) !important;
  color: var(--color-orange-700) !important;
  box-shadow: 0 2px 4px rgba(249, 115, 22, 0.15) !important;
  transform: translateY(-1px) !important;
}

.dark .flatpickr-am-pm:hover {
  background: var(--color-orange-900) !important;
  border-color: var(--color-orange-600) !important;
  color: var(--color-orange-300) !important;
}

