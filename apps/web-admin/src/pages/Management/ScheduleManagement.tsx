...
const formatDateVN = (dateString: string | Date | undefined | null): string => {
...
const ScheduleManagement: React.FC = () => {
  ...
  const [selectedScheduleForAction, setSelectedScheduleForAction] = useState<ScheduleItem | null>(
    null
  );
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const transformedSchedules = schedulesList.map((schedule: any) => {
          ...
          // Extract date from start_time if date is not available
          let date = schedule.date;
          if (!date && schedule.start_time) {
            // Extract date part from start_time datetime
      ...
      showToast(t('scheduleManagement.messages.loadError'), 'error');
  ...
  const getStatusText = (status: string) => {
    const statusMap: { [key: string]: string } = {
      SCHEDULED: t('scheduleManagement.status.scheduled'),
      IN_PROGRESS: t('scheduleManagement.status.inProgress'),
      COMPLETED: t('scheduleManagement.status.completed'),
      CANCELLED: t('scheduleManagement.status.cancelled'),
      POSTPONED: t('scheduleManagement.status.postponed'),
    };
  ...
  const filteredSchedules = useMemo(() => {
    return schedules.filter(schedule => {
      ...
      const matchesClassType =
        classTypeFilter === 'all' || schedule.gym_class?.category === classTypeFilter;

      // Date filter
      let matchesDate = true;
      if (dateFilter) {
        // Extract date from schedule.date or from start_time if date is not available
        const scheduleDateValue = schedule.date || schedule.start_time;
        if (scheduleDateValue) {
          const scheduleDate = new Date(scheduleDateValue);
          const filterDate = new Date(dateFilter);
          if (!isNaN(scheduleDate.getTime()) && !isNaN(filterDate.getTime())) {
            matchesDate =
  ...
  const getExportData = useCallback(() => {
    ...
    return exportSchedules.map(schedule => ({
      ...
      'Giờ bắt đầu': schedule.start_time
        ? formatVietnamDateTime(schedule.start_time, 'time')
        : 'N/A',
      ...
      'Ngày tạo': schedule.created_at
        ? formatVietnamDateTime(schedule.created_at, 'datetime')
        : 'N/A',
    }));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(t('scheduleManagement.messages.deleteSuccess'), 'success');
      await loadSchedules();
      setIsDeleteDialogOpen(false);
      setScheduleToDelete(null);
    } catch (error: any) {
      showToast(error.message || t('scheduleManagement.messages.deleteError'), 'error');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleSave = async (data: Partial<ScheduleItem>) => {
    try {
      if (selectedSchedule) {
        ...
        showToast(t('scheduleManagement.messages.updateSuccess'), 'success');
      } else {
        await scheduleService.createSchedule(data as any);
        showToast(t('scheduleManagement.messages.createSuccess'), 'success');
      }
      await loadSchedules();
      setIsFormModalOpen(false);
      setSelectedSchedule(null);
    } catch (error: any) {
      showToast(error.message || t('scheduleManagement.messages.saveError'), 'error');
      throw error;
    }
  };
  ...
  const STATUS_OPTIONS = [
    { value: 'all', label: t('scheduleManagement.filter.allStatuses') },
    { value: 'SCHEDULED', label: t('scheduleManagement.status.scheduled') },
    { value: 'IN_PROGRESS', label: t('scheduleManagement.status.inProgress') },
    { value: 'COMPLETED', label: t('scheduleManagement.status.completed') },
    { value: 'CANCELLED', label: t('scheduleManagement.status.cancelled') },
    { value: 'POSTPONED', label: t('scheduleManagement.status.postponed') },
  ];

  const CLASS_TYPE_OPTIONS = [
    { value: 'all', label: t('scheduleManagement.filter.allClassTypes') },
  ...
  return (
    <>
      ...
      <div className='p-3 space-y-3'>
          ...
          <div className='flex gap-2 flex-wrap'>
            ...
            {filteredSchedules.length > 0 && (
              <ExportButton
                ...
                filename={t('scheduleManagement.export.filename')}
              ...
              <div className='flex items-center gap-3'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <div className='text-theme-xs text-gray-500 dark:text-gray-400 font-inter leading-tight font-medium'>
                    {t('scheduleManagement.stats.totalSchedules')}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className='relative'>
              <div className='flex items-center gap-3'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <div className='text-theme-xs text-gray-500 dark:text-gray-400 font-inter leading-tight font-medium'>
                    {t('scheduleManagement.stats.scheduled')}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className='relative'>
              <div className='flex items-center gap-3'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <div className='text-theme-xs text-gray-500 dark:text-gray-400 font-inter leading-tight font-medium'>
                    {t('scheduleManagement.stats.inProgress')}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className='relative'>
              <div className='flex items-center gap-3'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <div className='text-theme-xs text-gray-500 dark:text-gray-400 font-inter leading-tight font-medium'>
                    {t('scheduleManagement.stats.cancelled')}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3'>
            ...
            <div>
              <CustomSelect
                ...
                placeholder={t('scheduleManagement.filter.allStatuses')}
            ...
            <div>
              <CustomSelect
                ...
                placeholder={t('scheduleManagement.filter.allClassTypes')}
                className='font-inter'
              />
            </div>
            ...
            <div className='relative'>
              ...
              {dateFilter && (
                <button
                  ...
                  title={t('scheduleManagement.filter.clearDateFilter')}
                >
        ...
        {/* Schedules List */}
        {isLoading ? (
          <TableLoading text={t('scheduleManagement.messages.loading')} />
        ) : filteredSchedules.length === 0 ? (
          <AdminCard>
            <div className='text-center py-12 text-gray-500 dark:text-gray-400 font-inter'>
              ...
              <p className='text-theme-sm font-heading text-gray-700 dark:text-gray-300'>
                {t('scheduleManagement.empty.noSchedules')}
              </p>
              <p className='text-theme-xs text-gray-500 dark:text-gray-400 mt-1'>
                {t('scheduleManagement.empty.createNew')}
              </p>
            </div>
          </AdminCard>
        ) : (
              ...
              <AdminTable>
                <AdminTableHeader>
                  <AdminTableRow>
                    ...
                    <AdminTableCell header>{t('scheduleManagement.table.room')}</AdminTableCell>
                    <AdminTableCell header>{t('scheduleManagement.table.dateTime')}</AdminTableCell>
                    <AdminTableCell header>{t('scheduleManagement.table.capacity')}</AdminTableCell>
                    <AdminTableCell header>{t('scheduleManagement.table.status')}</AdminTableCell>
                  </AdminTableRow>
                </AdminTableHeader>
                <AdminTableBody>
                      ...
                      >
                        ...
                        <AdminTableCell>
                          <div className='flex items-center gap-1.5'>
                            ...
                            <span className='text-theme-xs font-heading text-gray-900 dark:text-white group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors duration-200'>
                              {formatDateVN(item.date || item.start_time)}
                            </span>
                          </div>
            ...
            >
              ...
              <div className='py-1'>
                ...
                >
                  ...
                  <span>{t('scheduleManagement.actions.edit')}</span>
                </button>
                <button
                  onClick={() => {
                    setActionMenuOpen(false);
                ...
                >
                  ...
                  <span>{t('scheduleManagement.actions.delete')}</span>
                </button>
              </div>
            </div>
          </>
        ...
        <ConfirmDialog
          ...
          title={t('scheduleManagement.delete.confirmTitle')}
          message={t('scheduleManagement.delete.confirmMessage', {
            name: scheduleToDelete?.class_name || '',
          })}
          confirmText={t('scheduleManagement.actions.delete')}
